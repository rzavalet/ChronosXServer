...
{
  if (num_samples > 0) {
    return (float)cumulative_delay / num_samples;
  }
...
{
  if (desired_delay_bound > 0 && average_service_delay > desired_delay_bound) {
    return (float)(average_service_delay - desired_delay_bound) / desired_delay_bound;
  }
